...
const INSTRUCTIONS = [
  ...
  '입력: VOC 본문(또는 표준분류 유형명)과 채널. 먼저 VOC 분류 에이전트와 동일한 기준(6개 그룹·22개 표준분류·대응영역)으로 유형을 파악합니다.',
...
const children = [
  ...
  P('M365 Copilot 에이전트 빌더(선언적 에이전트)용 — 고객 셀프 해결 가이드 에이전트。 분류 에이전트(‘U+VOC 고객가이드’)와 같은 분류 지식을 공유하고, 출력만 ‘고객용 가이드’입니다.', { italics: true, color: '666666' }),
  ...
  B('**voc-classification-knowledge.docx** — 6그룹·22분류·대응영역 기준 및 유형별 응대 가이드(분류 에이전트와 공용)'),
