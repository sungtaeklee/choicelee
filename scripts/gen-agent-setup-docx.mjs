...
const children = [
  ...
  CHK('**지식 공통(중요)**: voc-service-knowledge.docx(서비스 이해) + voc-classification-knowledge.docx(분류 기준) + **voc-learning-examples.docx(실데이터 골든 예시 889건·신뢰도·분류사유 코드)**'),
  CHK('**분류 규칙**: 신뢰도 하 또는 분류사유 L01/L04 등(원문 짧음·다주제·감정 위주)이면 자동 확정하지 말고 **검토필요**로 표시'),
  CHK('**웹 지식 연결**: 가능하면 **www.uplus.co.kr** 을 지식 소스로 추가(서비스 최신 정보 근거). 사내 SharePoint·Teams·FAQ도 함께'),
  CHK('**저장 → 채팅에서 테스트 → 공유**(심사/동료 계정 추가) — M365 Copilot 라이선스 보유 사내 계정만 접근'),

  H2('에이전트 ① ‘U+VOC 고객가이드’ — 분류·분석'),
  P('지시문(요지): **먼저 U+one·홈페이지 서비스 구조·용어(서비스 지식)를 이해한 뒤**, 흩어진 VOC를 6그룹·22분류·대응영역으로 분류하고 요약·예상 응대문(고객/문자/담당자 메일)·개선 과제 생성. 근거 없으면 ‘확인 필요’. “사이트용” 요청 시 구조화 JSON 출력.', {}),
  P('추천 프롬프트:', { bold: true }),
