...
const children = [
  ...
  CHK('**지식 공통(중요)**: voc-service-knowledge.docx(U+one·홈페이지 서비스 이해 — 용어·화면구조) + voc-classification-knowledge.docx(4그룹·22분류·대응 가이드)'),
  ...
  P('지시문(요지): **먼저 U+one·홈페이지 서비스 구조·용어(서비스 지식)를 이해한 뒤**, 흩어진 VOC를 4그룹·22분류·대응영역으로 분류하고 요약·예상 응대문(고객/문자/담당자 메일)·개선 과제 생성. 근거 없으면 ‘확인 필요’. “사이트용” 요청 시 구조화 JSON 출력.', {}),
  P('추천 프롬프트:', { bold: true }),
