...
JSON 스키마:
{
  "summary": "고객이 무엇을 원하는지 행동 중심으로 한 문장(35자 내외, 완결된 종결어미로 끝내고 인사말·군더더기·추측 제외). 예: '로그인 인증문자가 안 와 로그인 불가, 복구 요청'",
...
/* 제공자 결정: LLM_PROVIDER가 유효하면 그것을, 없거나 오타면 "설정된 키"로 자동 감지.
   → LLM_PROVIDER를 빠뜨려도 OPENAI_*만 넣으면 동작(흔한 설정 실수 방지). */
function resolveProvider() {
  const p = String(process.env.LLM_PROVIDER || '').toLowerCase().trim()
  if (p === 'anthropic' || p === 'azure' || p === 'openai') return p
  if (process.env.OPENAI_API_KEY && process.env.OPENAI_BASE_URL && process.env.OPENAI_MODEL) return 'openai'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 실패해도 어떤 제공자로 시도했는지 함께 반환 → 환경변수 설정 디버깅 용이
    res.status(200).json({ ok: false, provider: resolveProvider(), error: String(e && e.message || e).slice(0, 300) })
  }
}
